...
import NearMatchService from './nearMatchService.js';
...
export const handleInputMessageService = (
  ...
  if (
    typeof word === 'string' &&
    chatMsg.toLowerCase().trim() === word.toLowerCase().trim()
  ) {
    ...
    const pIndex = users.findIndex(user => user.socketId === presenterSocketId);
    if (
      index !== -1 &&
      index < users.length &&
      pIndex !== -1 &&
    ...
    typeof word === 'string' &&
    (NearMatchService.matchPercentage(
      chatMsg.toLowerCase().trim(),
      word.toLowerCase().trim()
    ) >= 65 ||
      NearMatchService.isNearlyCorrect(
        chatMsg.toLowerCase().trim(),
        word.toLowerCase().trim()
      ))
  ) {
    ...
    // sending message in the lobby
    const moderator = 'Moderator';
    const msg = `${users[index].userName} nearly guessed the word`;
    io.to(roomId).emit('recievedChatData', {
      userName: moderator,
      chatMsg: msg,
      type: 'nearlyCorrect',
  ...
  if (chatBlock === true) {
    ...
    io.to(socketId).emit('recievedChatData', {
      userName: 'Moderator',
      chatMsg: 'You are blocked from chat',
      type: 'blocked',
    });
    return;
  }
